...
class MyWorks_WC_QBO_Sync_QBO_Lib {
	...
	public function __construct($queue_conn=false){
		if(!session_id()) {
			session_start();
		}
		...
		if(($queue_conn || !$this->option_checked('mw_wc_qbo_sync_pause_up_qbo_conection')) && $this->Context=='' && $this->realm==''){
			$this->creds();
			$this->connect();
		}
	...
	public function get_slmt_hstry_msg(){
		$mag = __( '<h2>Need more than '.$this->get_hd_ldys_lmt().' days of history? Upgrade to an annual plan!</h2>', 'mw_wc_qbo_sync' );
		if($this->is_plg_lc_p_l()){
			$mag = __( '<h2>Need more than '.$this->get_hd_ldys_lmt().' days of history? Upgrade to a paid plan!</h2>', 'mw_wc_qbo_sync' );
		}
	...
	public function qbo_functionality_after_pugin_activation(){
		...
		if(!$this->option_checked('mw_wc_qbo_sync_qb_func_af_plg_act_run') || $force_run){
			if($this->is_connected()){
				...
				$AccountService = new QuickBooks_IPP_Service_Account();				
				$accounts =	$AccountService->query($Context, $realm, "SELECT * FROM Account WHERE FullyQualifiedName IN('Sales Income','Sales of Product Income','Inventory Asset','Cost of Goods Sold') ");
				//$this->_p($accounts);
				if($accounts && is_array($accounts) && !empty($accounts)){
					foreach($accounts as $Account){
						...
						if($Account->getFullyQualifiedName() == 'Inventory Asset' && $Account->getAccountType() == 'Other Current Asset'){
							$asset_acc_ref = $this->qbo_clear_braces($Account->getId());
						}
						
						if($Account->getFullyQualifiedName() == 'Cost of Goods Sold' && $Account->getAccountType() == 'Cost of Goods Sold'){
				...
				if(is_array($map_data)){
					...
					foreach($map_data as $map_data_c){
						...
						if($QtyOnHand!=$_stock){
							...
							//
							if($this->option_checked('mw_wc_qbo_sync_invnt_pull_set_prd_stock_sts')){
		...
		if(is_array($wc_items) && count($wc_items)){
			...
			if($this->option_checked('mw_wc_qbo_sync_wolim_iqilid_desc')){
				$solm_arr = array(
					...
					'wc_cog_item_total_cost', 'reduced_stock', 'type',
	...
	public function get_wc_order_details_from_order($order_id,$order){
		...
		if($order_id && is_object($order) && !empty($order)){
			...
			$order_items = $this->get_data($wpdb->prepare("SELECT * FROM {$wc_oi_table} WHERE `order_id` = %d ",$order_id));
			//$this->_p($order_items);
			...
			if(is_array($line_items) && count($line_items)){
				foreach ( $line_items as $item ) {
					...
					$mqi_fwi = $this->get_mapped_qbo_items_from_wc_items($product_data,false,$acof_txt);
	...
	public function debug(){
		...
		//$this->_p($this->get_wc_order_details_from_order(9941,get_post(9941)));
	...
	public function get_cf_map_data(){
		...
		if(is_array($cmd) && count($cmd)){
			foreach($cmd as $row){
				...
				$rd[$row['wc_field'].'_ext_data'] = $row['ext_data'];
			}
		}
		return $rd;
	}
	...
	/**/
	private function gpltci_new($qbo_tax_code,$qbo_item,$tax_details,$qi_k,$qbo_inv_items){
		if(is_array($qbo_item) && is_array($tax_details)){
			global $wpdb;
			$rate_id = 0;
			if(isset($qbo_item['line_tax']) && $qbo_item['line_tax'] > 0 && isset($qbo_item['line_tax_data']) && !empty($qbo_item['line_tax_data'])){
				$line_tax = $qbo_item['line_tax'];
				//$line_subtotal_tax = $qbo_item['line_subtotal_tax'];
				foreach($tax_details as $tk => $tv){
					if($tv['rate_id'] > 0){
						$tax_rate = (float) $this->get_field_by_val($wpdb->prefix.'woocommerce_tax_rates','tax_rate','tax_rate_id',$tv['rate_id']);
						if($tax_rate > 0){
							$calc_tax_t = round($qbo_item['line_total']*$tax_rate/100,2);
							//$calc_tax_s = round($qbo_item['line_subtotal']*$tax_rate/100);
							if(floatval($line_tax) == floatval($calc_tax_t)){
								$rate_id = $tv['rate_id'];
								break;
							}
						}
	...
	public function get_per_line_tax_code_id($qbo_tax_code,$qbo_item,$tax_details,$qi_k=null,$qbo_inv_items=array(),$new_logic=true){
		if($new_logic){
			return $this->gpltci_new($qbo_tax_code,$qbo_item,$tax_details,$qi_k,$qbo_inv_items);
		}
		...
		if(is_array($qbo_item) && is_array($tax_details)){
			...
			if(isset($qbo_item['line_tax']) && $qbo_item['line_tax'] > 0 && isset($qbo_item['line_tax_data']) && !empty($qbo_item['line_tax_data'])){
				if(!is_null($qi_k)){
					if(isset($tax_details[$qi_k])){
						$rate_id = $tax_details[$qi_k]['rate_id'];
					}
				}else{
					$line_tax_data = @unserialize($qbo_item['line_tax_data']);
					if(is_array($line_tax_data) && !empty($line_tax_data)){
						$t_total = $line_tax_data['total'];
						foreach($tax_details as $tk => $tv){
							if($tv['tax_amount'] == $t_total){
								$rate_id = $tv['rate_id'];
								break;
							}
						}
	...
	public function get_qs_catch_all_order_ids(){
		...
		if($this->option_checked('mw_wc_qbo_sync_pause_up_qbo_conection')){
			...
			if(!empty($last_q_int_dt)){
				global $wpdb;
				$date_whr = " AND p.`post_date` >= '{$last_q_int_dt}' AND p.`post_date` <= '{$wp_date_time_c}' ";
	...
	public function get_wc_deposit_payment_list($date_whr,$gateway,$currency,$single_pmnt_id=0){
		...
		pm8.meta_id as payment_id, pm8.meta_value as transaction_id, pm9.meta_value as paid_date, pm10.meta_value as payment_method, pim.qbo_payment_id, pm12.meta_value as stripe_txn_fee, pm13.meta_value as paypal_txn_fee, pm11.meta_value as order_number_formatted, pm_con.meta_value as _alg_wc_custom_order_number
		FROM
		{$wpdb->prefix}posts as p
	...
	public function get_wc_deposit_sr_order_list($date_whr,$gateway,$currency,$single_ord_id=0){
		$date_whr = str_replace('`{date}`','p.post_date',$date_whr);
		//$date_whr = str_replace('`{date}`','pm9.meta_value',$date_whr);
			...
			if(is_array($p_maps_data) && count($p_maps_data)){
				foreach($p_maps_data as $pmd){
					...
					if($ps_order_status!=''){
						...
						$p_list_arr = $this->get_wc_deposit_sr_order_list($date_whr,$wc_paymentmethod,$payment_cur,$single_ord_id);
					}
			...
			*/
			...
			if($is_res_ok && $response['body']!=''){
				...
				if(isset($creds['oauth_access_token']) && isset($creds['oauth_access_token_secret']) && isset($creds['encryption_key'])){
					if(isset($creds['oauth_access_token_d']) && isset($creds['oauth_access_token_secret_d'])){
...
EOF;
	...
	//31-03-2017
	public function get_push_invoice_map_data($item_arr,$is_qbosa_sr=false){
		if(is_array($item_arr) && count($item_arr)){
			$item_ids = implode(',',$item_arr);
			if($this->is_connected()){
				$Context = $this->Context;
	...
	//
	public function count_woocommerce_product_list($search_txt='',$is_inventory=false,$p_type='') {
		$status = 'publish';
		global $wpdb;
		$search_txt = $this->sanitize($search_txt);
	...
	public function get_woocommerce_product_list($search_txt='',$limit='',$is_inventory=false,$p_type='') {
		$status = 'publish';
		global $wpdb;
		$search_txt = $this->sanitize($search_txt);
	...
	//07-03-2017
	public function count_woocommerce_variation_list($search_txt='',$is_inventory=false,$stock_status='') {
		global $wpdb;
	...
	public function get_woocommerce_variation_list($search_txt='',$is_inventory=false,$limit='',$stock_status='') {
		global $wpdb;
	...
	public function get_qbo_invoice_list($search_txt='',$limit='',$date_from='',$date_to='',$qb_customer_id){
		if($this->is_connected() && !$this->is_plg_lc_p_l()){
			...
			$sql = "SELECT * FROM Invoice WHERE Id > '0' $whr ORDER BY Id DESC $limit ";
			//echo $sql;
	...
	public function get_qb_customer_invoice_pdf($qb_customer_id,$qbo_inv_id){
		if($this->is_connected() && !$this->is_plg_lc_p_l()){
			$qb_customer_id = (int) $qb_customer_id;
			$qbo_inv_id = (int) $qbo_inv_id;
	...
	public function get_order_sync_to_qbo_as($order_id){
		...
		if($q_osa!= 'Invoice' && $q_osa!= 'SalesReceipt'){
			$q_osa = 'Invoice';
		}
		//echo $q_osa;
		return $q_osa;
