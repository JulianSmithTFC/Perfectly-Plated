				...
				if($sync_type=='push'){
					if($item_type=='customer'){
						...
						if($return_id){
							...
							if($manual_push_update){
								$msg = "<span class='error_red'>There was an error updating customer #$id ,please check log</span>";
							}else{
								$msg = "<span class='error_red'>There was an error pushing customer #$id ,please check log</span>";
							}							
						}
					...
					if($item_type=='vendor'){
						...
						if($return_id){
							...
							if($manual_push_update){
								$msg = "<span class='error_red'>There was an error updating vendor #$id ,please check log</span>";
							}else{
								$msg = "<span class='error_red'>There was an error pushing vendor #$id ,please check log</span>";
							}							
						}
						$MSQS_QL->show_sync_window_message($key, $msg , $per, $tot_item);
					}
					...
					if($item_type=='invoice'){											
						...
						if($return_id){
							...
							$yith_gp_fc = false;
							//$manual_push_update
							if(!$MSQS_QL->is_order_sync_as_sales_receipt($id) && $MSQS_QL->option_checked('mw_wc_qbo_sync_compt_yithwgcp_gpc_ed')){
							...
							if($MSQS_QL->option_checked('mw_wc_qbo_sync_order_as_sales_receipt') || $MSQS_QL->is_order_sync_as_sales_receipt($id)){
								if($manual_push_update){
									$msg = "<span class='success_green'>Order #$ord_id_num has been updated, QuickBooks salesreceipt id #$return_id</span>";
								}else{
									$msg = "<span class='success_green'>Order #$ord_id_num has been pushed, QuickBooks salesreceipt id #$return_id</span>";
								}								
							}else{
								if($manual_push_update){
									$msg = "<span class='success_green'>Order #$ord_id_num has been updated, QuickBooks invoice id #$return_id</span>";
								}else{
									$msg = "<span class='success_green'>Order #$ord_id_num has been pushed, QuickBooks invoice id #$return_id</span>";
								}								
							}						
						}else{
							if($MSQS_QL->option_checked('mw_wc_qbo_sync_order_as_sales_receipt') || $MSQS_QL->is_order_sync_as_sales_receipt($id)){
								if($manual_push_update){
									$msg = "<span class='error_red'>There was an error updating salesreceipt #$ord_id_num ,please check log</span>";
								}else{
									$msg = "<span class='error_red'>There was an error pushing order #$ord_id_num ,please check log</span>";
								}
							}else{
								if($manual_push_update){
									$msg = "<span class='error_red'>There was an error updating invoice #$ord_id_num ,please check log</span>";
								}else{
									$msg = "<span class='error_red'>There was an error pushing order #$ord_id_num ,please check log</span>";
								}
							}
						...
						if($yith_gp_fc){
							if($return_id_gift_payment){
								...
								$msg .= "<span class='error_red'>There was an error pushing gift payment for order #$ord_id_num ,please check log</span>";
							}
						}
					...
					if($item_type=='product'){									
						...
						if($return_id){
							...
							if($manual_push_update){
								$msg = "<span class='error_red'>There was an error updating product #$id ,please check log</span>";
							}else{
								$msg = "<span class='error_red'>There was an error pushing product #$id ,please check log</span>";
							}							
						}
						$MSQS_QL->show_sync_window_message($key, $msg , $per, $tot_item);
					}
					...
					if($item_type=='variation'){							
						...
						if($return_id){
							...
							if($manual_push_update){
								$msg = "<span class='error_red'>There was an error updating variation #$id ,please check log</span>";
							}else{
								$msg = "<span class='error_red'>There was an error pushing variation #$id ,please check log</span>";
							}							
						}
						$MSQS_QL->show_sync_window_message($key, $msg , $per, $tot_item);
					}
					...
					if($item_type=='payment' && !$MSQS_QL->option_checked('mw_wc_qbo_sync_order_as_sales_receipt')){
						...
						if($return_id){
							...
							$msg = "<span class='error_red'>There was an error pushing payment #$id ,please check log</span>";
						}
						$MSQS_QL->show_sync_window_message($key, $msg , $per, $tot_item);				
					}
					...
					if($item_type=='category'){						
						...
						if($return_id){
							...
							if($manual_push_update){
								$msg = "<span class='error_red'>There was an error updating category #$id ,please check log</span>";
							}else{
								$msg = "<span class='error_red'>There was an error pushing category #$id ,please check log</span>";
							}							
						}
						$MSQS_QL->show_sync_window_message($key, $msg , $per, $tot_item);
					}
					...
					if($item_type=='inventory'){
						...
						if($return_id){
							...
							$msg = "<span class='error_red'>There was an error updating inventory #$id ,please check log</span>";
						}
						$MSQS_QL->show_sync_window_message($key, $msg , $per, $tot_item);
					}
					...
					if($item_type=='v_inventory'){
						...
						if($return_id){
							...
							$msg = "<span class='error_red'>There was an error updating variation inventory #$id ,please check log</span>";
						}
						$MSQS_QL->show_sync_window_message($key, $msg , $per, $tot_item);
					}
					...
					if($item_type=='refund'){
						...
						if($return_id){
							...
							if($manual_push_update){
								$msg = "<span class='error_red'>There was an error updating refund #$id ,please check log</span>";
							}else{
								$msg = "<span class='error_red'>There was an error pushing refund #$id ,please check log</span>";
							}							
						}
						$MSQS_QL->show_sync_window_message($key, $msg , $per, $tot_item);
					}
				...
				if($sync_type=='pull' && $MSQS_QL->option_checked('mw_wc_qbo_sync_pull_enable')){
					...
					if($item_type=='product'){
						...
						if($return_id){
							...
							if($manual_pull_update){
								$msg = "<span class='error_red'>There was an error importing product #$id ,please check log</span>";
							}else{
								$msg = "<span class='error_red'>There was an error updating product #$id ,please check log</span>";
							}							
						}
					...
					if($item_type=='category' && !$MSQS_QL->is_plg_lc_p_l()){
						...
						if($return_id){
							...
							if($manual_pull_update){
								...
								$msg = "<span class='error_red'>There was an error importing category #$id ,please check log</span>";
							}							
						}
						$MSQS_QL->show_sync_window_message($key, $msg , $per, $tot_item);
					}
