...
class MyWorks_WC_QBO_Sync_Admin {
		...
		if(is_array($queue_data) && count($queue_data)){
			...
			foreach($queue_data as $q_dt){
				...
				if($item_type=='Product' && $item_action=='ProductPush'){
					...
					$prd_log_count++;
					$run = 1;
					if($this->mw_qbo_wc_product_save($item_id)){
						$success = 1;
					}
					$wpdb->query($wpdb->prepare("UPDATE {$wpdb->prefix}mw_wc_qbo_sync_real_time_sync_queue SET `run` = %d , `success` = %d WHERE `id` = %d",1,$success,$row_id));
				}
	...
	public function wc_order_list_custom_footer_js(){
		...
		if(is_array($wco_ids) && count($wco_ids) && $MSQS_QL->if_show_sync_status(count($wco_ids))){
			//queue changes
			if($MSQS_QL->option_checked('mw_wc_qbo_sync_pause_up_qbo_conection')){
				$MSQS_QL = new MyWorks_WC_QBO_Sync_QBO_Lib(true);	
			}
	...
	public static function qbo_admin_sync_connection_submenu(){
		$cn_fn = (isset($_GET['new']) && $_GET['new']=='1')?'myworks-wc-qbo-sync-admin-connection-new.php':'myworks-wc-qbo-sync-admin-connection-design.php';
		$cn_fn = (isset($_GET['latest']) && $_GET['latest']=='1')?'myworks-wc-qbo-sync-admin-connection-latest.php':$cn_fn;
		//$cn_fn = (isset($_GET['design']) && $_GET['design']=='1')?'myworks-wc-qbo-sync-admin-connection-design.php':$cn_fn;
...
EOF;
	...
	public function mw_qbo_sync_logging($ext_param){
		...
		if(is_array($ext_param)){
			if($ext_param['is_auto_inventory_pull_active'] || $ext_param['is_deposit_sync_active']){
				$crn_msg_co = false;
			}
		}
		...
		if(defined('DISABLE_WP_CRON')){
			if( DISABLE_WP_CRON ){
				if($crn_msg_co || get_option('mw_wc_qbo_sync_cron_notice_status') != 'true'){
	...
	public function qbo_setup_admin_notice_wp_cron(){
		...
		echo '<div title="MyWorks QuickBooks Sync Error" class="notice cross-relative-icon notice-error mwqs-setup-notice">'.__("MyWorks Woo Sync for QuickBooks Online needs to enable WP CRON for queue syncing, deposit syncing, auto inventory import email logging, data refresh etc.<br/> Please remove code like <b>define('DISABLE_WP_CRON', true)</b> from <b>wp-config.php (if found)</b> and add <b>define('DISABLE_WP_CRON', false)</b>.", 'mw_wc_qbo_sync').'<a '.$a_style.' href="'.$current_page.'" class="cross_icon"></a></div>';
	}
	...
	public function myworks_wc_qbo_sync_pu_variation_stock_update($post_ID, $post_after, $post_before){		
		...
		if($post_ID>0 && is_object($post_after) && !empty($post_after)){
			if(isset($post_after->post_type) && $post_after->post_type == 'product_variation'){
				$product_id = $post_ID;
				if(is_object($post_before) && !empty($post_before)){
